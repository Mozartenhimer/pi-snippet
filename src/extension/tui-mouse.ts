 ...
 * 3. The visible viewport is the last `terminal.rows` lines of the buffer, so a
 *    screen row maps to a line index by counting from the bottom.
 *
...
export class ClickableText {
	...
	private handleInput(data: string): { consume?: boolean } | undefined {
		...
		while ((match = SGR_MOUSE.exec(data)) !== null) {
			...
			const hit = this.hitTest(row, col);
			if (hit) this.options.onActivate(hit);
			else this.options.onMiss?.();
		}
		return sawMouse ? { consume: true } : undefined;
	}

	/** Maps a screen cell to a target, or null. Exposed for tests. */
	hitTest(screenRow: number, screenCol: number): ClickTarget | null {
		...
		const line = this.lines[viewportTop + screenRow];
