...
- Use it when you've just offered a short list of named options as the answer to a question, the bare name of each option is a complete reply and may be wrapped on its own.
...
- The wrapped text must read as a grammatical continuation of your sentence; the chips replace nothing and the message must read naturally as prose. For the user, the text is just underlined.
...
(These are good seperate options the user may want you to do)
### binary question:

Shall I <${tagName}>proceed</${tagName}>?

(proceed provides an unambguis yes or no to your question)

### Committing

Want me to <${tagName}>commit</${tagName}> and <${tagName}>push</${tagName}>?

(These are options that provide an affirmative answer to the question)
