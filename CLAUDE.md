...
**Suggestions come from two layers, and layer 1 wins.** `<snippet>` tags parsed from the message are layer 1. When a finished message asks something and carries no tags, `extension/magic.ts` sends it to a small model, which returns anchor/reply pairs (`shared/inferred.ts` decides what to believe of them). A message with even one tag is never sent — the layers must never compete for the same sentence. Both the anchor and the reply are dropped unless they appear verbatim in the message's non-code text: a paraphrasing model costs a chip, it never produces a wrong one. The reply rule is the load-bearing one — while it was free text, a live small model answered `Should it stream or buffer?` with the reply `Stream the input.` on a perfectly verbatim anchor, i.e. it picked a side for the user. Quoting removes the ability to invent instead of asking the model not to use it. A corollary in `parseInferred`: an anchor the model reuses across entries is re-seated onto each entry's own reply span, because a repeated anchor means it pointed at the question rather than the options, and dropping the duplicate would silently discard a branch.
