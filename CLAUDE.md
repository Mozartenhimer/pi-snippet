...
python3 scripts/osc8-probe.py alacritty   # the same, in the environment a real Alacritty exports
...
bash scripts/xvfb-env.sh               # a display for the window harness below, on a headless box
DISPLAY=:99 python3 scripts/alacritty-click.py  # real Alacritty window, real pointer: chip, click, insertion
...
`readme-demo.py` is the odd one out: it asserts nothing, it *records*. Same pty and the same `mock-llm.js` playing both models, with `asciinema rec` between the pty and pi and the pacing slowed to reading speed. Its scenario is pi-snippet itself — someone asking how the chip numbers work, answered by a model that marks its own offers of what to do next, with the three files the rules live in left untagged for the second model to find. The `.pi/skills/snippet-demo` skill's Wizard of Cardboard was recorded first and dropped: it is the right thing to run by hand and the wrong thing to open a README with, where the reader is still deciding what this is. Two things the recording needs that a test does not: `TERM_PROGRAM=ghostty`, or pi-tui reports `hyperlinks: false` and the recording shows the bare-label path instead of chips; and `--cwd` pointed at a checkout of `main`, because pi's footer paints the working directory's git branch into every frame.
...
**The second model is asked once per turn, at `agent_settled` — not at every `message_end`.** A tool-calling turn ends several assistant messages before the agent stops, and the ones in the middle are not points the user can reply at: a chip painted on "let me check the build log" scrolls away behind the tool output before anyone can click it, and each one spent a request. So `message_end` only parks the message in `pendingInference` and `agent_settled` sends whichever one the turn stopped on. `agent_settled` rather than `agent_end`, which pi fires once per agent *loop*: a run that retries, compacts, or picks up a queued message fires it several times and only the last is the end of the turn. The footer's `not sent` therefore covers the whole turn, tool calls included, rather than flickering to a count between hops. Every test that expects an inference must fire `agent_settled` after its `message_end`; a `message_end` alone is the middle of a turn.
...
- **Alacritty sets no `TERM_PROGRAM`, in any version, so pi-tui's Alacritty entry never fires.** The terminal itself is fine — its default hint carries `hyperlinks = true` and `command = xdg-open`, so a `pisnip://` OSC 8 URL is matched by the hyperlink rather than by the hint's scheme regex, and a click hands it over verbatim. The gesture there is a **plain left click**: Alacritty's default hint sets no mouse modifiers, so unlike Ghostty and VTE it needs no Ctrl (a drag still selects rather than activating). What is missing is only the handshake — `TERM_PROGRAM` is what pi-tui looks for and Alacritty exports `ALACRITTY_WINDOW_ID` instead, and `TERM` falls back to `xterm-256color` whenever the alacritty terminfo entry is absent, so it is no substitute. `PI_HYPERLINKS=1` makes a session work today; the real fix is one entry upstream. Measured by `scripts/alacritty-click.py`, whose `--stock` regime asserts the gap so that it fails the day pi-tui closes it.
