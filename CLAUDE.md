...
**Suggestions come from one layer.** The only source of chips is `<snippet>` tags parsed from the message. A client-side inference layer (a small model reading untagged questions — PRD §17) existed and was removed: ongoing per-message cost for second-class chips (no number, no keyboard path). Fix tagging via the prompt contract, not a second model. Removal rationale in PRD §17; the pattern worth stealing if it ever comes back — verbatim anchors or nothing, cache by message text, stand down on dead credentials.
