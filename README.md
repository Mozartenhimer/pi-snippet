...
- **Ctrl+click a chip** to insert it. The click is resolved by the terminal itself — the chip's href is a real `pisnip://` URL, and the desktop dispatches it back to the pi session that painted it. No terminal-wide mouse mode is ever engaged: the scroll wheel and text selection are never taken away. One-time setup: the first chip of a session offers to register the handler, and `/snippets` → *Register click handler* does the same thing whenever you want it (Linux; needs a terminal that paints OSC 8 hyperlinks — Ghostty, kitty, WezTerm, …). **If your chips are inert, it is almost certainly detection, not the terminal**: pi-tui recognises kitty but not Alacritty or gnome-terminal, both of which resolve a click perfectly once `PI_HYPERLINKS=1` is set in the environment pi runs in. Konsole ignores OSC 8 hyperlinks itself and cannot be made to work this way. `docs/linux-terminals.md` has the measurements, per terminal, and the gesture each one wants — Alacritty needs no modifier at all, and kitty wants plain click or Ctrl+Shift rather than Ctrl.
