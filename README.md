...
- **`/snippets`** toggles three things independently: the feature, the `Alt` shortcuts, and click-to-insert. Each choice is remembered — the toggles are written to `$XDG_CONFIG_HOME/pi-snippet/settings.json` (override the location with `PI_SNIPPET_SETTINGS`) and read back at startup, so click-to-insert does not have to be switched on again every session. `--no-suggestions` disables everything for one session without touching what is stored.
...
| Settings | `src/extension/settings.ts` | The three `/snippets` toggles on disk, outside the session store: preferences about the tool, not state of one conversation. A missing or malformed file falls back to defaults, and a failed write is reported rather than silently promised. |
