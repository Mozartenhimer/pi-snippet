...
Suggestions come from two layers, painted as one. Layer 1 is the model's own tags. Layer 2 is a second, small model — OpenRouter's `qwen/qwen3.7-flash` by default, changeable in `/snippets`, `PI_SNIPPET_MODEL` overriding for a session — which reads each finished message (tags included, told to keep them and add more) and returns it with additional tags. Its chips are indistinguishable from layer-1 chips: numbered, `Alt+N` addressable, click-to-insert. It only runs on messages that ask something, caches its answers, and stands down after three consecutive failures. When it cannot run or a call dies, the footer says so (`snippet: second model unavailable` / `snippet: second model failed`) — the message itself never gains error markup, just no extra chips. A question the primary model failed to tag still gets its chips; a message that asks nothing costs nothing.
